...
void GetResult(int n, int value, int count, vector<int> numbers) {
    ...
    // number[i]의 자리수까지만 체크해본다
    while (numbers[n] / int(pow(10, i - 1)) != 0) {
        // 234에서 2번째 수를 구하고 싶다면, 234 % 10^2 를 한 결과를 10^(2-1)로 나눠야 3이라는 값을 얻을 수 있다.
        int ith_number = (numbers[i] % int(pow(10, i))) / pow(10, i - 1);
        int ith_value = (value % int(pow(10, i))) / pow(10, i - 1);
    ...
    if (possible_flag) {
        GetResult(n+1, value + numbers[i], count + 1, numbers);
    }
